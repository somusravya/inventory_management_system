...
void stock::withd(int qty)
{
	if(quant>=qty)
	{
		...
		cout<<"\n\nStock updated.\n";
		st.total_bill=pr*qty;

		cout<<"\n\nTotal price to be paid:"<<st.total_bill;
	ielse 
	   cout<<"\n\nInsufficient stock";
	    getch();	
}
...
void Bill_receipt(){
	fout.open("shop.dat",ios::binary|ios::out);
	fout<<"\t\t\tBILL\t\t\t";
	fout<<"product name"<<"\t"<<"price\t\t"<<"total_bill";
	fout<<st.name<<"\t"<<st.pr<<"\t"<<st.total_bill<<"\t";
	fout.close();
...
else
{
	...
	if (i==1)
	{
	withdraw();Bill_receipt();getch();goto custmenu;
	}
	else if(i==2)
	{
		system("cls");
